...
suite_filter_options = ("dimensions: 2,3,5,10,20,40 " +  # skip dimension 40
                        #"dimensions: 40 " +
    ...
    while evalsleft() > 0 and not problem.final_target_hit:
        ...
        if fmin is scipy.optimize.fmin:
            ...
            opts = {'maxfevals': evalsleft(),
                    #'CSA_invariant_path': True, #Affine invariant path
                    'CSA_squared': True, #C^1 stepsize change
                    'CSA_damp_mueff_exponent': 1, #change exponent of mueff in d_sigma
            ...
            es.adapt_sigma.damps += 1 - es.adapt_sigma.cs
            es.adapt_sigma.cs = 1 # to set no cumulation for the stepsize


            xopt, es = fmin(problem, es, None, #propose_x0(), sigma,
                            restarts=0,
